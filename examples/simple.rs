...
fn main() {
    ...
    println!(
        "sswap : {:?}",
        rgsl::blas::level1::sswap(&mut tmp_vec, &mut tmp_vec2)
    );
    ...
    println!(
        "Simple Airy::Ai test : {}",
        rgsl::airy::Ai(0.5f64, rgsl::Mode::PrecDouble)
    );
    println!("Simple Bessel::I0 test : {}", rgsl::bessel::I0(0.5f64));
    println!(
        "Simple Legendre::conical test : {}",
        rgsl::legendre::conical::half(0.37f64, 1.2f64)
    );
    println!(
        "Simple Legendre::conical test : {}",
        rgsl::legendre::conical::half(0.37f64, 1.2f64)
    ...
    println!(
        "Simple BLAS::level1::snrm2 test : {}",
        rgsl::blas::level1::snrm2(&tmp_vec)
    );
    println!(
        "Simple Logarithm::log test : {}",
        rgsl::logarithm::log(0.1f64)
    );
    ...
    rgsl::blas::level2::sger(
        1.7f32,
        &tmp_vec,
        &rgsl::VectorF32::from_slice(&[0.4f32, 14f32, 3f32, 2f32]).unwrap(),
        &mut tmp_mat,
    );
    println!("=> Simple BLAS level2 test after :\n{:?}", tmp_mat);
    println!(
        "\nSimple CBLAS level1 test : {}",
        rgsl::cblas::level1::sdsdot(1i32, 0.6f32, &[1.1f32], 1i32, &[2.07f32], 1i32)
    );
    ...
    println!(
        "Simple Trigonometric test sf_hypot(1.0) : {}",
        1f64.sf_hypot()
    );
