...
fn main() {
    ...
    let mut tmp_vec = rgsl::VectorFloat::from_slice([1f32, 0f32, 3f32, 2f32]).unwrap();
    let mut tmp_vec2 = rgsl::VectorFloat::from_slice([14f32, 6f32, -3f32, 1.2f32]).unwrap();
    ...
    println!("\n=== MatrixFloat tests ===");
    let mut tmp_mat = rgsl::MatrixFloat::new(2u64, 3u64).unwrap();
    ...
    println!("\n=== Complex tests ===");
    let mut tmp_complex : rgsl::Complex = Default::default();
    ...
    tmp_mat = rgsl::MatrixFloat::new(4u64, 4u64).unwrap();
    tmp_mat.set(1, 2, 42f32);
    tmp_mat.set(0, 0, 1f32);
    tmp_mat.set(1, 2, 3f32);
    tmp_mat.set(1, 3, 0.5f32);
    println!("\n=> Simple BLAS level2 test before :\n{}", tmp_mat);
    rgsl::blas::level2::sger(1.7f32, &tmp_vec, &rgsl::VectorFloat::from_slice([0.4f32, 14f32, 3f32, 2f32]).unwrap(), &mut tmp_mat);
