...
pub fn dir_2d(r: &mut Rng, x: &mut f64, y: &mut f64) {
    unsafe { sys::gsl_ran_dir_2d(ffi::FFI::unwrap_unique(r), x, y) }
}
...
pub fn dir_2d_trig_method(r: &mut Rng, x: &mut f64, y: &mut f64) {
    unsafe { sys::gsl_ran_dir_2d_trig_method(ffi::FFI::unwrap_unique(r), x, y) }
}
...
pub fn dir_3d(r: &mut Rng, x: &mut f64, y: &mut f64, z: &mut f64) {
    unsafe { sys::gsl_ran_dir_3d(ffi::FFI::unwrap_unique(r), x, y, z) }
}
...
pub fn dir_nd(r: &mut Rng, x: &mut [f64]) {
    unsafe {
        sys::gsl_ran_dir_nd(ffi::FFI::unwrap_unique(r), x.len() as usize, x.as_mut_ptr())
    }
}
