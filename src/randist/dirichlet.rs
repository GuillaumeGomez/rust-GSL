...
pub fn dirichlet_pdf(alpha: &[f64], theta: &[f64]) -> f64 {
    unsafe { ffi::randist::gsl_ran_dirichlet_pdf(alpha.len(), alpha.as_ptr(), theta.as_ptr()) }
}
...
pub fn dirichlet_lnpdf(alpha: &[f64], theta: &[f64]) -> f64 {
    unsafe { ffi::randist::gsl_ran_dirichlet_lnpdf(alpha.len(), alpha.as_ptr(), theta.as_ptr()) }
}
