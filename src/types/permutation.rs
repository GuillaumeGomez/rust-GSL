...
impl Permutation {
    ...
    pub fn copy(&self, dest: &mut Permutation) -> Value {
        Value::from(unsafe { sys::gsl_permutation_memcpy(dest.p, self.p) })
    }
    ...
    pub fn swap(&mut self, i: usize, j: usize) -> Value {
        Value::from(unsafe { sys::gsl_permutation_swap(self.p, i, j) })
    }
    ...
    pub fn inverse(&self, inv: &mut Permutation) -> Value {
        Value::from(unsafe { sys::gsl_permutation_inverse(inv.p, self.p) })
    }
    ...
    pub fn next(&mut self) -> Value {
        Value::from(unsafe { sys::gsl_permutation_next(self.p) })
    }
    ...
    pub fn prev(&mut self) -> Value {
        Value::from(unsafe { sys::gsl_permutation_prev(self.p) })
    }
    ...
    pub fn permute_vector(&mut self, v: &mut VectorF64) -> Value {
        Value::from(unsafe { sys::gsl_permute_vector(self.p, v.unwrap_unique()) })
    }
    ...
    pub fn permute_vector_inverse(&self, v: &mut VectorF64) -> Value {
        Value::from(unsafe { sys::gsl_permute_vector_inverse(self.p, v.unwrap_unique()) })
    }

    #[cfg(feature = "v2_2")]
    pub fn permute_matrix(&self, A: &mut MatrixF64) -> Value {
        Value::from(unsafe { sys::gsl_permute_matrix(self.p, A.unwrap_unique()) })
    }

    pub fn permute_matrix_float(&self, A: &mut MatrixF32) -> Value {
        Value::from(unsafe { sys::gsl_permute_matrix_float(self.p, A.unwrap_unique()) })
    }

    pub fn permute_matrix_complex(&self, A: &mut MatrixComplexF64) -> Value {
        Value::from(unsafe { sys::gsl_permute_matrix_complex(self.p, A.unwrap_unique()) })
    }

    pub fn permute_matrix_complex_float(&self, A: &mut MatrixComplexF32) -> Value {
        Value::from(unsafe { sys::gsl_permute_matrix_complex_float(self.p, A.unwrap_unique()) })
    }
    ...
    pub fn mul(&mut self, pa: &Permutation, pb: &Permutation) -> Value {
        Value::from(unsafe { sys::gsl_permutation_mul(self.p, pa.p, pb.p) })
    }
    ...
    pub fn linear_to_canonical(&self, q: &mut Permutation) -> Value {
        Value::from(unsafe { sys::gsl_permutation_linear_to_canonical(q.p, self.p) })
    }
    ...
    pub fn canonical_to_linear(&self, p: &mut Permutation) -> Value {
        Value::from(unsafe { sys::gsl_permutation_canonical_to_linear(p.p, self.p) })
    }
