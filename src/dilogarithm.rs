...
/// 
/// Note that Abramowitz & Stegun refer to the Spence integral S(x)=Li_2(1-x) as the dilogarithm rather than Li_2(x).
pub fn dilog_e(x: f64) -> (enums::GslValue, gsl::Result) {
...
pub fn complex_dilog_e(r: f64, theta: f64) -> (enums::GslValue, gsl::Result, gsl::Result) {
    let mut result = unsafe { zeroed::<::ffi::gsl_sf_result>() };
    ...
    (ret, gsl::Result{val: result.val, err: result.err}, gsl::Result{val: result_im.val, err: result_im.err})
}