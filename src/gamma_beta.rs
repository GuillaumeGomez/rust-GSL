...
pub mod gamma {
    ...
    /// The maximum value of x such that \Gamma(x) is not considered an overflow is given by the macro gsl::GSL_SF_GAMMA_XMAX and is 171.0.
    pub fn gamma(x: f64) -> f64 {
        unsafe { ffi::gsl_sf_gamma(x) }
    }

    /// This routine provides an exponential function \exp(x) using GSL semantics and error checking.
    pub fn gamma_e(x: f64) -> (enums::GslValue, gsl::Result) {
