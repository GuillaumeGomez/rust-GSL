...
pub fn wtss_m(w: &[f64], wstride: usize, data: &[f64], stride: usize, wmean: f64) -> f64 {
    unsafe { sys::gsl_stats_wtss_m(w.as_ptr(), wstride, data.as_ptr(), stride, data.len() as _, wmean) }
}
...
pub fn wvariance_m(w: &[f64], wstride: usize, data: &[f64], stride: usize, wmean: f64) -> f64 {
    unsafe { sys::gsl_stats_wvariance_m(w.as_ptr(), wstride, data.as_ptr(), stride, data.len() as _, wmean) }
}

pub fn wabsdev_m(w: &[f64], wstride: usize, data: &[f64], stride: usize, wmean: f64) -> f64 {
    unsafe { sys::gsl_stats_wabsdev_m(w.as_ptr(), wstride, data.as_ptr(), stride, data.len() as _, wmean) }
}

pub fn wskew_m_sd(w: &[f64], wstride: usize, data: &[f64], stride: usize, wmean: f64, wsd: f64) -> f64 {
    unsafe { sys::gsl_stats_wskew_m_sd(w.as_ptr(), wstride, data.as_ptr(), stride, data.len() as _, wmean, wsd) }
}

pub fn wkurtosis_m_sd(w: &[f64], wstride: usize, data: &[f64], stride: usize, wmean: f64, wsd: f64) -> f64 {
    unsafe { sys::gsl_stats_wkurtosis_m_sd(w.as_ptr(), wstride, data.as_ptr(), stride, data.len() as _, wmean, wsd) }
}

pub fn pvariance(data1: &[f64], stride1: usize, data2: &[f64], stride2: usize) -> f64 {
    unsafe { sys::gsl_stats_pvariance(data1.as_ptr(), stride1, data1.len() as _, data2.as_ptr(), stride2, data2.len() as _) }
}

pub fn ttest(data1: &[f64], stride1: usize, data2: &[f64], stride2: usize) -> f64 {
    unsafe { sys::gsl_stats_ttest(data1.as_ptr(), stride1, data1.len() as _, data2.as_ptr(), stride2, data2.len() as _) }
}
...
pub fn gsl_stats_minmax_index(data: &[f64], stride: usize) -> (usize, usize) {
    ...
    unsafe { sys::gsl_stats_minmax_index(&mut min, &mut max, data.as_ptr(), stride, data.len() as _) }
    (min, max)
}
...
#[cfg(feature = "v2_5")]
pub fn median(data: &mut [f64], stride: usize, k: usize) -> f64 {
    unsafe { sys::gsl_stats_median(data.as_mut_ptr(), stride, data.len() as _, k) }
}
